...
async def and_gate_direct(dut):
    ...
    for i in range(50): 
        ...
        if c_output.value.integer != c: 
            cocotb.log.error(f"[TEST] incorrect value of C {c_output.value} when a = {a} and b = {b} expected {c}")
        else:
            cocotb.log.info(f"[TEST] correct value of C {c_output.value} when a = {a} and b = {b}")
